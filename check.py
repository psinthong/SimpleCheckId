...
start_time = datetime(2018, 4, 21, 13, 30, 0, tzinfo=pytz.utc)
end_time = datetime(2018, 4, 22, 19, 40, 0, tzinfo=pytz.utc)
...
def index():
    ...
    if now < start_time:
        diff = start_time - now
        return "This service will not be available before %s. %d mins left." % \
               (start_time.astimezone(tz).strftime("%Y-%m-%d %H:%M"), diff.total_seconds()/60)
    elif now > end_time:
...
def init():
    with open('midterm1-seats.txt', 'r') as f:
        for line in f:
            parts = line.split('\t')
